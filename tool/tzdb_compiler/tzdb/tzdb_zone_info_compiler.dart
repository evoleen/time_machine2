...
class TzdbZoneInfoCompiler {
  ...
  factory TzdbZoneInfoCompiler() =>
      TzdbZoneInfoCompiler.log(TextWriter((String text) => print(text)));
    ...
    if (source.contains(_zone1970TabFile)) {
      var iso3166Dict = {
        for (var bits in iso3166)
          bits[0]:
              TzdbZone1970LocationCountry(/*name:*/ bits[1], /*code:*/ bits[0])
      };
      database.zone1970Locations = source
          .readLines(_zone1970TabFile)
          .where((line) => line != '' && !line.startsWith("#"))
          .map((line) =>
  ...
  String _inferVersion(FileSource source) {
    ...
    if (match != null) {
      ...
      _log?.WriteLine(
          'Inferred version $version from file/directory name ${source.origin}');
      return version;
    }
