...
class TzdbZoneInfoParser {
  ...
  Offset _nextOffset(Tokens tokens, String name) =>
      ParserHelper.parseOffset(_nextString(tokens, name));
  ...
  String? _nextOptional(Tokens tokens, String name) =>
      ParserHelper.ParseOptional(_nextString(tokens, name));
  ...
  ZoneYearOffset parseDateTimeOfYear(Tokens tokens, bool forRule) {
    ...
    return ZoneYearOffset(mode, monthOfYear, dayOfMonth, dayOfWeek,
        advanceDayOfWeek, timeOfDay, addDay);
  }
      ...
      default:
        if (keyword.isEmpty) {
          if (previousZone == null) {
            // InvalidDataException
            throw Exception(
                'Zone continuation provided with no previous zone line');
          }
  ...
  static int parseMonth(String text) {
    ...
    Preconditions.checkArgument(
        text.isNotEmpty, 'text', "Value must not be empty");
  ...
  ZoneLine parseZone(String name, Tokens tokens) {
    ...
    return ZoneLine(
        name, offset, rules, format, year, ZoneYearOffset.StartOfYear);
  }
