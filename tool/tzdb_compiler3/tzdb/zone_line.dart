...
@immutable
class ZoneLine {
  static final OffsetPattern _percentZPattern =
      OffsetPattern.createWithInvariantCulture('i');

  /// Initializes a new instance of the [ZoneLine] class.
  const ZoneLine(this.name, this.standardOffset, this.rules, this.format,
...
// #region IEquatable<Zone> Members
  ...
  @override
  bool operator ==(Object other) => other is ZoneLine && equals(other);
...
// #endregion
  ...
  @override
  int get hashCode {
    var hashables = [name, standardOffset, rules, format, untilYear];
  ...
  ZoneRuleSet resolveRules(Map<String, List<RuleLine>> allRules) {
    ...
    if (allRules.containsKey(rules)) {
      ...
      for (var zoneRecurrenceRules
          in ruleSet.map((x) => x.GetRecurrences(this))) {
        _rules.addAll(zoneRecurrenceRules);
      }
      return ZoneRuleSet.rules(
          _rules, standardOffset, untilYear, untilYearOffset);
    } else {
      try {
