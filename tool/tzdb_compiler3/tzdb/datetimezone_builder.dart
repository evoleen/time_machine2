...
class DateTimeZoneBuilder {
  ...
  static DateTimeZone build(String id, List<ZoneRuleSet> ruleSets) {
    Preconditions.checkArgument(ruleSets.isNotEmpty, 'ruleSets',
  ...
  DateTimeZone _buildZone(String id, List<ZoneRuleSet> ruleSets) {
    ...
    if (_zoneIntervals.length == 1 && _tailZone == null) {
      return FixedDateTimeZone(
          id, _zoneIntervals[0].wallOffset, _zoneIntervals[0].name);
    } else {
  ...
  void _addIntervals(ZoneRuleSet ruleSet) {
    ...
    var firstRule = lastZoneInterval == null
        ? null
        : lastOrDefault((activeRules
                .map((rule) =>
                    ...
                    {
                      'rule': rule,
                      'prev': rule.previousOrSame(
                          start,
                          lastZoneInterval.standardOffset,
                          lastZoneInterval.savings)
                    })
                .where((pair) => pair['prev'] != null)
                .toList()
              ..sort((a, b) => (a['prev'] as Transition)
                  .instant
                  .compareTo((b['prev'] as Transition).instant)))
    ...
    while (true) {
      ...
      for (int i = 0; i < activeRules.length; i++) {
        ...
        var nextTransition = rule.next(previousTransition.instant,
      ...
      if (ruleSet.isInfinite && activeRules.length == 2) {
        ...
        if (startRule.isInfinite && endRule.isInfinite) {
          ...
          _tailZone = StandardDaylightAlternatingMap(
              standardOffset, startRule, endRule);
        }
      }
    }
  }
  ...
  void _coalesceIntervals() {
    for (int i = 0; i < _zoneIntervals.length - 1; i++) {
      ...
      if (current.name == next.name &&
        ...
        _zoneIntervals[i] =
            IZoneInterval.withEnd(current, IZoneInterval.rawEnd(next))!;
