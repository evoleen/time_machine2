...
class OffsetPatternParser implements IPatternParser<Offset> {
  static final Map<String /*char*/,
          CharacterHandler<Offset, _OffsetParseBucket>>
    ...
    ':': (pattern, builder) => builder.addLiteral1(
        ...
        IParseResult.timeSeparatorMismatch /**<Offset>*/),
    'h': (pattern, builder) => throw IInvalidPatternError.format(
        TextErrorMessages.hour12PatternNotSupported, ['Offset']),
    'H': SteppedPatternBuilder.handlePaddedField<Offset, _OffsetParseBucket>(
        ...
        23,
        _getPositiveHours,
        (bucket, value) => bucket.hours = value),
    'm': SteppedPatternBuilder.handlePaddedField<Offset, _OffsetParseBucket>(
        ...
        59,
        _getPositiveMinutes,
        (bucket, value) => bucket.minutes = value),
    's': SteppedPatternBuilder.handlePaddedField<Offset, _OffsetParseBucket>(
        ...
        59,
        _getPositiveSeconds,
        (bucket, value) => bucket.seconds = value),
    '+': _handlePlus,
    '-': _handleMinus,
    'Z': (ignored1, ignored2) =>
        throw InvalidPatternError(TextErrorMessages.zPrefixNotAtStartOfPattern)
  };
  ...
  static int _getPositiveHours(Offset offset) =>
      offset.inMilliseconds.abs() ~/ TimeConstants.millisecondsPerHour;

  static int _getPositiveMinutes(Offset offset) =>
      (offset.inMilliseconds.abs() % TimeConstants.millisecondsPerHour) ~/
      TimeConstants.millisecondsPerMinute;

  static int _getPositiveSeconds(Offset offset) =>
      (offset.inMilliseconds.abs() % TimeConstants.millisecondsPerMinute) ~/
  ...
  @override
  IPartialPattern<Offset> parsePattern(
          String patternText, TimeMachineFormatInfo formatInfo) =>
      _parsePartialPattern(patternText, formatInfo);

  IPartialPattern<Offset> _parsePartialPattern(
      String patternText, TimeMachineFormatInfo formatInfo) {
    ...
    if (patternText.length == 1) {
      switch (patternText) {
        case 'g':
          return ICompositePatternBuilder.buildAsPartial(
              CompositePatternBuilder<Offset>()
                ..add(
                    _parsePartialPattern(
                        formatInfo.offsetPatternLong, formatInfo),
                    (offset) => true)
                ...
                ..add(
                    _parsePartialPattern(
                        formatInfo.offsetPatternMedium, formatInfo),
                    (arg) => _hasZeroSeconds(arg))
                ..add(
                    _parsePartialPattern(
                        formatInfo.offsetPatternShort, formatInfo),
                    (arg) => _hasZeroSecondsAndMinutes(arg)));
        case 'G':
        ...
        case 'i':
          return ICompositePatternBuilder.buildAsPartial(
              CompositePatternBuilder<Offset>()
                ..add(
                    _parsePartialPattern(
                        formatInfo.offsetPatternLongNoPunctuation, formatInfo),
                    (offset) => true)
                ..add(
                    _parsePartialPattern(
                        formatInfo.offsetPatternMediumNoPunctuation,
                        formatInfo),
                    (arg) => _hasZeroSeconds(arg))
                ..add(
                    _parsePartialPattern(
                        formatInfo.offsetPatternShortNoPunctuation, formatInfo),
                    (arg) => _hasZeroSecondsAndMinutes(arg)));
        case 'I':
        ...
        default:
          throw IInvalidPatternError.format(
              TextErrorMessages.unknownStandardFormat, [patternText, 'Offset']);
      }
    }
    ...
    var patternBuilder = SteppedPatternBuilder<Offset, _OffsetParseBucket>(
        formatInfo, () => _OffsetParseBucket());
    patternBuilder.parseCustomPattern(
        zPrefix ? patternText.substring(1) : patternText,
        _patternCharacterHandlers);
    // No need to validate field combinations here, but we do need to do something a bit special
    // for Z-handling.
    IPartialPattern<Offset> pattern =
        patternBuilder.build(Offset.hoursAndMinutes(5, 30));
  ...
  static bool _hasZeroSeconds(Offset offset) =>
      (offset.inSeconds % TimeConstants.secondsPerMinute) == 0;

  /// Returns true if the offset is representable just in hours (no minutes or seconds).
  static bool _hasZeroSecondsAndMinutes(Offset offset) =>
      (offset.inSeconds % TimeConstants.secondsPerHour) == 0;

  // #region Character handlers
  static void _handlePlus(PatternCursor pattern,
      SteppedPatternBuilder<Offset, _OffsetParseBucket> builder) {
    builder.addField(PatternFields.sign, pattern.current);
    builder.addRequiredSign((bucket, positive) => bucket.isNegative = !positive,
        (offset) => offset.inMilliseconds >= 0);
  }
...
class _ZPrefixPattern implements IPartialPattern<Offset> {
  ...
  @override
  ParseResult<Offset> parse(String text) => text == 'Z'
      ? ParseResult.forValue<Offset>(Offset.zero)
      : _fullPattern.parse(text);

  @override
  String format(Offset value) =>
      value == Offset.zero ? 'Z' : _fullPattern.format(value);
  ...
  StringBuffer appendFormat(Offset value, StringBuffer builder) {
    ...
    return value == Offset.zero
        ? (builder..write('Z'))
        : _fullPattern.appendFormat(value, builder);
  }
}
...
class _OffsetParseBucket extends ParseBucket<Offset> {
  ...
  ParseResult<Offset> calculateValue(PatternFields usedFields, String text) {
    int totalSeconds = hours * TimeConstants.secondsPerHour +
