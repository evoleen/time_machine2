...
class _MonthFormatActionHolder<TResult, TBucket extends ParseBucket<TResult>>
    extends IPostPatternParseFormatAction<TResult> {
  final int _count;
  ...
  Function(TResult, StringBuffer) buildFormatAction(PatternFields finalFields) {
    ...
    List<String> textValues = _count == 3
        ? (genitive
            ? _formatInfo.shortMonthGenitiveNames
            : _formatInfo.shortMonthNames)
        : (genitive
            ? _formatInfo.longMonthGenitiveNames
            : _formatInfo.longMonthNames);
...
abstract class DatePatternHelper {
  ...
  static CharacterHandler<TResult, TBucket>
      createYearOfEraHandler<TResult, TBucket extends ParseBucket<TResult>>(
      ...
      switch (count) {
        case 2:
          ...
          builder.addFormatLeftPad(
              2, (value) => ((yearGetter(value) % 100) + 100) % 100,
              assumeNonNegative: true, assumeFitsInCount: true);
        ...
        case 4:
          ...
          builder.addFormatLeftPad(4, yearGetter,
              assumeNonNegative: false, assumeFitsInCount: true);
          break;
        default:
          throw IInvalidPatternError.format(
              TextErrorMessages.invalidRepeatCount, [pattern.current, count]);
      }
    };
  ...
  static CharacterHandler<TResult, TBucket>
      createMonthOfYearHandler<TResult, TBucket extends ParseBucket<TResult>>(
          int Function(TResult) numberGetter,
          Function(TBucket, int) textSetter,
          Function(TBucket, int) numberSetter) {
    return (PatternCursor pattern,
        SteppedPatternBuilder<TResult, TBucket> builder) {
      int count = pattern.getRepeatCount(4);
      PatternFields field;
      switch (count) {
        ...
        case 2:
          ...
          builder.addParseValueAction(
              count, 2, pattern.current, 1, 99, numberSetter);
          builder.addFormatLeftPad(count, numberGetter,
              assumeNonNegative: true, assumeFitsInCount: count == 2);
          break;
        case 3:
        case 4:
          field = PatternFields.monthOfYearText;
          var format = builder.formatInfo;
          List<String> nonGenitiveTextValues =
              count == 3 ? format.shortMonthNames : format.longMonthNames;
          List<String> genitiveTextValues = count == 3
              ? format.shortMonthGenitiveNames
              : format.longMonthGenitiveNames;
          if (nonGenitiveTextValues == genitiveTextValues) {
            builder.addParseLongestTextAction(pattern.current, textSetter,
                format.compareInfo, nonGenitiveTextValues);
          } else {
            builder.addParseLongestTextAction(pattern.current, textSetter,
                format.compareInfo, genitiveTextValues, nonGenitiveTextValues);
          }
          ...
          builder.addPostPatternParseFormatAction(
              _MonthFormatActionHolder<TResult, TBucket>(
                  format, count, numberGetter));
          break;
        default:
  ...
  static CharacterHandler<TResult, TBucket>
      createDayHandler<TResult, TBucket extends ParseBucket<TResult>>(
          int Function(TResult) dayOfMonthGetter,
          int Function(TResult) dayOfWeekGetter,
          Function(TBucket, int) dayOfMonthSetter,
          Function(TBucket, int) dayOfWeekSetter) {
    return (pattern, builder) {
      ...
      switch (count) {
        ...
        case 2:
          ...
          // Handle real maximum value in the bucket
          builder.addParseValueAction(
              count, 2, pattern.current, 1, 99, dayOfMonthSetter);
          builder.addFormatLeftPad(count, dayOfMonthGetter,
              assumeNonNegative: true, assumeFitsInCount: count == 2);
          break;
        case 3:
        case 4:
          ...
          List<String?> textValues =
              count == 3 ? format.shortDayNames : format.longDayNames;
          builder.addParseLongestTextAction(
              pattern.current, dayOfWeekSetter, format.compareInfo, textValues);
          builder.addFormatAction(
              (value, sb) => sb.write(textValues[dayOfWeekGetter(value)]));
          break;
        default:
          throw StateError('Invalid count!');
      }
  ...
  static CharacterHandler<TResult, TBucket>
      createEraHandler<TResult, TBucket extends ParseBucket<TResult>>(
          Era Function(TResult) eraFromValue,
          /*LocalDatePatternParser.*/ LocalDateParseBucket Function(TBucket)
              dateBucketFromBucket) {
    return (pattern, builder) {
      ...
      builder.addFormatAction((value, sb) =>
          sb.write(formatInfo.getEraPrimaryName(eraFromValue(value))));
    };
  }

  /// Creates a character handler for the calendar specifier (c).
  static CharacterHandler<TResult, TBucket>
      createCalendarHandler<TResult, TBucket extends ParseBucket<TResult>>(
          CalendarSystem Function(TResult) getter,
          Function(TBucket, CalendarSystem) setter) {
    return (pattern, builder) {
