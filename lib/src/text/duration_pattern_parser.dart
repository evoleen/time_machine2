...
class TimePatternParser implements IPatternParser<Time> {
  static final Map< /*char*/ String, CharacterHandler<Time, _TimeParseBucket>>
    ...
    '.': TimePatternHelper.createPeriodHandler<Time, _TimeParseBucket>(
        9,
        _getPositiveNanosecondOfSecond,
        (bucket, value) => bucket.addNanoseconds(value)),
    ':': (pattern, builder) => builder.addLiteral1(
        builder.formatInfo.timeSeparator,
        IParseResult.timeSeparatorMismatch /**<Span>*/),
    'D': _createDayHandler(),
    'H': _createTotalHandler(PatternFields.hours24,
        TimeConstants.nanosecondsPerHour, TimeConstants.hoursPerDay, 402653184),
    'h': _createPartialHandler(PatternFields.hours24,
        TimeConstants.nanosecondsPerHour, TimeConstants.hoursPerDay),
    'M': _createTotalHandler(
        PatternFields.minutes,
        TimeConstants.nanosecondsPerMinute,
        TimeConstants.minutesPerDay,
        24159191040),
    'm': _createPartialHandler(PatternFields.minutes,
        TimeConstants.nanosecondsPerMinute, TimeConstants.minutesPerHour),
    'S': _createTotalHandler(
        PatternFields.seconds,
        TimeConstants.nanosecondsPerSecond,
        TimeConstants.secondsPerDay,
        1449551462400),
    's': _createPartialHandler(PatternFields.seconds,
        TimeConstants.nanosecondsPerSecond, TimeConstants.secondsPerMinute),
    'f': TimePatternHelper.createFractionHandler<Time, _TimeParseBucket>(
        9,
        _getPositiveNanosecondOfSecond,
        (bucket, value) => bucket.addNanoseconds(value)),
    'F': TimePatternHelper.createFractionHandler<Time, _TimeParseBucket>(
  ...
  IPattern<Time> parsePattern(
    ...
    var patternBuilder = SteppedPatternBuilder<Time, _TimeParseBucket>(
        formatInfo, () => _TimeParseBucket());
    ...
    return patternBuilder.build(Time(hours: 1) +
        Time(minutes: 30) +
        Time(seconds: 5) +
  ...
  static int _getPositiveNanosecondOfSecond(Time time) {
    return ITime.nanosecondOfDurationDay(time).abs() %
        TimeConstants.nanosecondsPerSecond;
  }

  static CharacterHandler<Time, _TimeParseBucket> _createTotalHandler(
      PatternFields field,
      int nanosecondsPerUnit,
      int unitsPerDay,
      int maxValue) {
    return (pattern, builder) {
      ...
      builder.addParseInt64ValueAction(count, 13, pattern.current, 0, maxValue,
          (bucket, value) => bucket.addUnits(value, nanosecondsPerUnit));
      builder.addFormatAction((Time value, StringBuffer sb) =>
          FormatHelper.leftPadNonNegativeInt64(
              _getPositiveNanosecondUnits(
                  value, nanosecondsPerUnit, unitsPerDay),
              count,
              sb));
    };
  }
  ...
  static CharacterHandler<Time, _TimeParseBucket> _createDayHandler() {
    return (pattern, builder) {
      ...
      builder.addField(PatternFields.totalTime, pattern.current);
      builder.addParseValueAction(count, 8, pattern.current, 0, 16777216,
          (bucket, value) => bucket.addDays(value));
  ...
  static CharacterHandler<Time, _TimeParseBucket> _createPartialHandler(
    ...
    return (pattern, builder) {
      ...
      builder.addFormatLeftPad(
          count,
          (time) => (((ITime.nanosecondOfDurationDay(time).abs() ~/
                  nanosecondsPerUnit)) %
              unitsPerContainer),
  ...
  static void _handlePlus(PatternCursor pattern,
      SteppedPatternBuilder<Time, _TimeParseBucket> builder) {
    builder.addField(PatternFields.sign, pattern.current);
    builder.addRequiredSign((bucket, positive) => bucket.isNegative = !positive,
        (time) => IInstant.trusted(time).epochDay >= 0);
  }

  static void _handleMinus(PatternCursor pattern,
      SteppedPatternBuilder<Time, _TimeParseBucket> builder) {
    builder.addField(PatternFields.sign, pattern.current);
    builder.addNegativeOnlySign(
        (bucket, positive) => bucket.isNegative = !positive,
        (time) => IInstant.trusted(time).epochDay >= 0);
  }

  static int _getPositiveNanosecondUnits(
      Time time, int nanosecondsPerUnit, int unitsPerDay) {
    ...
    if (floorDays >= 0) {
      return floorDays * unitsPerDay +
          ITime.nanosecondOfEpochDay(time) ~/ nanosecondsPerUnit;
    } else {
      ...
      int negativeValue = nanosecondOfDay == 0
          ...
          : (floorDays + 1) * unitsPerDay +
              nanosecondOfDay ~/ nanosecondsPerUnit;
...
class _TimeParseBucket extends ParseBucket<Time> {
  static final BigInt _bigIntegerNanosecondsPerDay =
  ...
  ParseResult<Time> calculateValue(PatternFields usedFields, String text) {
    ...
    if (_currentNanos < ITime.minNanoseconds ||
        _currentNanos > ITime.maxNanoseconds) {
      return IParseResult.forInvalidValuePostParse<Time>(
          text, TextErrorMessages.overallValueOutOfRange, ['Time']);
    }
