...
class AnnualDatePatternParser implements IPatternParser<AnnualDate> {
  ...
  static final Map<String /*char*/,
          CharacterHandler<AnnualDate, AnnualDateParseBucket>>
      _patternCharacterHandlers = {
    '%': SteppedPatternBuilder
        .handlePercent /**<AnnualDate, AnnualDateParseBucket>*/,
    '\'': SteppedPatternBuilder
        .handleQuote /**<AnnualDate, AnnualDateParseBucket>*/,
    '\"': SteppedPatternBuilder
        .handleQuote /**<AnnualDate, AnnualDateParseBucket>*/,
    '\\': SteppedPatternBuilder
        .handleBackslash /**<AnnualDate, AnnualDateParseBucket>*/,
    '/': (pattern, builder) => builder.addLiteral1(
        builder.formatInfo.dateSeparator,
        IParseResult.dateSeparatorMismatch /**<AnnualDate>*/),
    'M': DatePatternHelper.createMonthOfYearHandler<AnnualDate,
            AnnualDateParseBucket>(
        (value) => value.month,
  ...
  @override
  IPattern<AnnualDate> parsePattern(
      ...
      switch (patternText[0]) {
        ...
        default:
          throw IInvalidPatternError.format(
              ...
              [patternText[0], 'AnnualDate']);
      }
    }

    var patternBuilder =
        SteppedPatternBuilder<AnnualDate, AnnualDateParseBucket>(
            formatInfo, () => AnnualDateParseBucket(_templateValue));
    patternBuilder.parseCustomPattern(patternText, _patternCharacterHandlers);
    patternBuilder.validateUsedFields();
    return patternBuilder.build(_templateValue);
  }

  static void _handleDayOfMonth(PatternCursor pattern,
      SteppedPatternBuilder<AnnualDate, AnnualDateParseBucket> builder) {
    ...
    switch (count) {
      ...
      case 2:
        ...
        builder.addParseValueAction(count, 2, pattern.current, 1, 99,
            (bucket, value) => bucket.dayOfMonth = value);
        builder.addFormatLeftPad(count, (value) => value.day,
...
class AnnualDateParseBucket extends ParseBucket<AnnualDate> {
  ...
  @override
  ParseResult<AnnualDate> calculateValue(
    ...
    if (day > CalendarSystem.iso.getDaysInMonth(2000, monthOfYearNumeric)) {
      return IParseResult.dayOfMonthOutOfRangeNoYear<AnnualDate>(
          text, day, monthOfYearNumeric);
    }

    return ParseResult.forValue<AnnualDate>(
        AnnualDate(monthOfYearNumeric, day));
  }
  ...
  ParseResult<AnnualDate>? _determineMonth(
      PatternFields usedFields, String text) {
    var x = usedFields &
        (PatternFields.monthOfYearNumeric | PatternFields.monthOfYearText);
    ...
    if (monthOfYearNumeric > CalendarSystem.iso.getMonthsInYear(2000)) {
      return IParseResult.isoMonthOutOfRange<AnnualDate>(
          text, monthOfYearNumeric);
    }
    return null;
  }
}
