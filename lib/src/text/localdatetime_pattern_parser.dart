...
class LocalDateTimePatternParser implements IPatternParser<LocalDateTime> {
  ...
  static final Map<String /*char*/,
          CharacterHandler<LocalDateTime, LocalDateTimeParseBucket>>
      _patternCharacterHandlers = {
    '%': SteppedPatternBuilder
        .handlePercent /**<LocalDateTime, LocalDateTimeParseBucket>*/,
    '\'': SteppedPatternBuilder
        .handleQuote /**<LocalDateTime, LocalDateTimeParseBucket>*/,
    '\"': SteppedPatternBuilder
        .handleQuote /**<LocalDateTime, LocalDateTimeParseBucket>*/,
    '\\': SteppedPatternBuilder
        .handleBackslash /**<LocalDateTime, LocalDateTimeParseBucket>*/,
    '/': (pattern, builder) => builder.addLiteral1(
        builder.formatInfo.dateSeparator,
        IParseResult.dateSeparatorMismatch /**<LocalDateTime>*/),
    'T': (pattern, builder) => builder.addLiteral2(
        'T', IParseResult.mismatchedCharacter /**<LocalDateTime>*/),
    'y': DatePatternHelper.createYearOfEraHandler<LocalDateTime,
            LocalDateTimeParseBucket>((value) => value.yearOfEra,
        (bucket, value) => bucket.date.yearOfEra = value),
    'u': SteppedPatternBuilder
        .handlePaddedField /**<LocalDateTime, LocalDateTimeParseBucket>*/
        (4, PatternFields.year, -9999, 9999, (value) => value.year,
            (bucket, value) => bucket.date.year = value),
    'M': DatePatternHelper.createMonthOfYearHandler<LocalDateTime,
            LocalDateTimeParseBucket>(
        (value) => value.monthOfYear,
        (bucket, value) => bucket.date.monthOfYearText = value,
        (bucket, value) => bucket.date.monthOfYearNumeric = value),
    'd': DatePatternHelper.createDayHandler<LocalDateTime,
            LocalDateTimeParseBucket>(
        (value) => value.dayOfMonth,
        (value) => value.dayOfWeek.value,
        (bucket, value) => bucket.date.dayOfMonth = value,
        (bucket, value) => bucket.date.dayOfWeek = value),
    '.': TimePatternHelper.createPeriodHandler<LocalDateTime,
            LocalDateTimeParseBucket>(9, (value) => value.nanosecondOfSecond,
        (bucket, value) => bucket.time.fractionalSeconds = value),
    ';': TimePatternHelper.createCommaDotHandler<LocalDateTime,
            LocalDateTimeParseBucket>(9, (value) => value.nanosecondOfSecond,
        (bucket, value) => bucket.time.fractionalSeconds = value),
    ':': (pattern, builder) => builder.addLiteral1(
        builder.formatInfo.timeSeparator,
        IParseResult.timeSeparatorMismatch /**<LocalDateTime>*/),
    'h': SteppedPatternBuilder.handlePaddedField<LocalDateTime,
            LocalDateTimeParseBucket>(
        2,
    ...
    'm': SteppedPatternBuilder.handlePaddedField<LocalDateTime,
            LocalDateTimeParseBucket>(
        2,
    ...
    's': SteppedPatternBuilder.handlePaddedField<LocalDateTime,
            LocalDateTimeParseBucket>(
        2,
    ...
    'f': TimePatternHelper.createFractionHandler<LocalDateTime,
            LocalDateTimeParseBucket>(9, (value) => value.nanosecondOfSecond,
        (bucket, value) => bucket.time.fractionalSeconds = value),
    'F': TimePatternHelper.createFractionHandler<LocalDateTime,
            LocalDateTimeParseBucket>(9, (value) => value.nanosecondOfSecond,
        (bucket, value) => bucket.time.fractionalSeconds = value),
    't': TimePatternHelper.createAmPmHandler<LocalDateTime,
            LocalDateTimeParseBucket>(
        (time) => time.hourOfDay, (bucket, value) => bucket.time.amPm = value),
    'c': DatePatternHelper.createCalendarHandler<LocalDateTime,
            LocalDateTimeParseBucket>((value) => value.calendar,
        (bucket, value) => bucket.date.calendar = value),
    'g': DatePatternHelper.createEraHandler<LocalDateTime,
            LocalDateTimeParseBucket>(
  ...
  @override
  IPattern<LocalDateTime> parsePattern(
    ...
    if (patternText.length == 1) {
      ...
      String? newPatternText =
          _expandStandardFormatPattern(patternCharacter, formatInfo);
      if (newPatternText == null) {
        throw IInvalidPatternError.format(
            TextErrorMessages.unknownStandardFormat,
            [patternCharacter, 'LocalDateTime']);
      }
      patternText = newPatternText;
    }

    var patternBuilder = SteppedPatternBuilder<LocalDateTime,
            LocalDateTimeParseBucket>(formatInfo,
  ...
  String? _expandStandardFormatPattern(
      /*char*/ String patternCharacter, TimeMachineFormatInfo formatInfo) {
    switch (patternCharacter) {
      case 'f':
        return formatInfo.dateTimeFormat.longDatePattern +
            ' ' +
            formatInfo.dateTimeFormat.shortTimePattern;
      case 'F':
      ...
      case 'G':
        return formatInfo.dateTimeFormat.shortDatePattern +
            ' ' +
            formatInfo.dateTimeFormat.longTimePattern;
      default:
...
class LocalDateTimeParseBucket extends ParseBucket<LocalDateTime> {
  ...
  LocalDateTimeParseBucket(
      LocalDate templateValueDate, LocalTime templateValueTime)
      : date = /*LocalDatePatternParser.*/
            LocalDateParseBucket(templateValueDate),
        time = /*LocalTimePatternParser.*/
            LocalTimeParseBucket(templateValueTime);
  ...
  /// and [ZonedDateTime].
  static ParseResult<LocalDateTime> combineBuckets(
    ...
    ParseResult<LocalDate> dateResult = dateBucket.calculateValue(
        usedFields & PatternFields.allDateFields, text);
    if (!dateResult.success) {
      return dateResult.convertError<LocalDateTime>();
    }
    ParseResult<LocalTime> timeResult = timeBucket.calculateValue(
        usedFields & PatternFields.allTimeFields, text);
  ...
  @override
  ParseResult<LocalDateTime> calculateValue(
