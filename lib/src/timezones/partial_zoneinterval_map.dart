...
class PartialZoneIntervalMap {
  ...
  PartialZoneIntervalMap(this.start, this.end, this._map) {
    ...
    Preconditions.debugCheckArgument(
        start <= end, 'end', 'Invalid start/end combination: $start - $end');
  }
  ...
  factory PartialZoneIntervalMap.forSingleZoneInterval(String name,
          Instant start, Instant end, Offset wallOffset, Offset savings) =>
      PartialZoneIntervalMap.forZoneInterval(
          IZoneInterval.newZoneInterval(name, start, end, wallOffset, savings));
  ...
  factory PartialZoneIntervalMap.forZoneInterval(ZoneInterval interval) =>
      PartialZoneIntervalMap(IZoneInterval.rawStart(interval),
          IZoneInterval.rawEnd(interval), SingleZoneIntervalMap(interval));

  ZoneInterval getZoneInterval(Instant instant) {
    Preconditions.debugCheckArgument(instant >= start && instant < end,
        'instant', 'Value $instant was not in the range [$start, $end)');
  ...
  bool get _isSingleInterval =>
      IZoneInterval.rawEnd(_map.getZoneInterval(start)) >= end;
  ...
  static ZoneIntervalMap convertToFullMap(
      Iterable<PartialZoneIntervalMap> maps) {
    ...
    for (var next in maps) {
      if (current == null) {
        current = next;
        Preconditions.debugCheckArgument(
            current.start == IInstant.beforeMinValue,
            'maps',
            "First partial map must start at the beginning of time");
        continue;
      }
      Preconditions.debugCheckArgument(
          current.end == next.start, 'maps', "Maps must abut");

      if (next.start == next.end) {
        continue;
      }

      var lastIntervalOfCurrent =
          current.getZoneInterval(current.end - Time.epsilon);
      var firstIntervalOfNext = next.getZoneInterval(next.start);

      if (!IZoneInterval.equalIgnoreBounds(
          lastIntervalOfCurrent, firstIntervalOfNext)) {
        ...
        if (current._isSingleInterval && next._isSingleInterval) {
          current = PartialZoneIntervalMap.forZoneInterval(
              IZoneInterval.withEnd(lastIntervalOfCurrent, next.end)!);
          ...
          coalescedMaps.add(PartialZoneIntervalMap.forZoneInterval(
              IZoneInterval.withEnd(
                  lastIntervalOfCurrent, firstIntervalOfNext.end)!));
          ...
          current = PartialZoneIntervalMap.forZoneInterval(
              IZoneInterval.withStart(
                  firstIntervalOfNext, lastIntervalOfCurrent.start)!);
        } else {
    ...
    Preconditions.debugCheckArgument(
        current != null, 'maps', "Collection of maps must not be empty");
    Preconditions.debugCheckArgument(current!.end == IInstant.afterMaxValue,
        'maps', "Collection of maps must end at the end of time");
