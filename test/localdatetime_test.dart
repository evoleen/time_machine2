...
void ToDateTimeUnspecified() {
  ...
  DateTime expected =
      DateTime(2011, 3, 5, 1, 0, 0); //, DateTimeKind.Unspecified);
...
void PeriodUntilSince() {
  ...
  var delta =
      const Period(months: 10, days: 30, hours: 4, minutes: 1, seconds: 1);
  ...
  if (Platform.isWeb) {
    var ldt = LocalDateTime(year, 1, 1, 13, 15, 55).addNanoseconds(
        TimeConstants.nanosecondsPerSecond -
            1); //.PlusNanoseconds(TimeConstants.nanosecondsPerSecond - 1);
    var expected =
        DateTime(year, 1, 1, 13, 15, 55 /*, DateTimeKind.Unspecified*/).add(
            const Duration(
                milliseconds: TimeConstants.millisecondsPerSecond - 1));
    ...
    var expected =
        DateTime(year, 1, 1, 13, 15, 55 /*, DateTimeKind.Unspecified*/).add(
            const Duration(
                microseconds: TimeConstants.microsecondsPerSecond - 1));
    var actual = ldt.toDateTimeLocal();
    expect(actual, expected);
  }
}
...
void FromDateTime_WithCalendar() {
  ...
  LocalDateTime expected =
      LocalDateTime(2011, 08, 05, 20, 53, 0, calendar: CalendarSystem.julian);
...
void TimeProperties_AfterEpoch() {
  ...
  LocalDateTime ldt = LocalDateTime(
          GregorianYearMonthDayCalculator.maxGregorianYear, 1, 2, 15, 48, 25)
  ...
  expect(
      15 * TimeConstants.microsecondsPerHour +
          48 * TimeConstants.microsecondsPerMinute +
          25 * TimeConstants.microsecondsPerSecond +
          123456 /*7*/,
      ldt.clockTime.timeSinceMidnight.inMicroseconds);
  expect(
      15 * TimeConstants.nanosecondsPerHour +
          48 * TimeConstants.nanosecondsPerMinute +
          25 * TimeConstants.nanosecondsPerSecond +
          123456789,
      ldt.clockTime.timeSinceMidnight.inNanoseconds);
...
void TimeProperties_BeforeEpoch() {
  ...
  LocalDateTime ldt = LocalDateTime(
          GregorianYearMonthDayCalculator.minGregorianYear, 1, 2, 15, 48, 25)
      .addNanoseconds(123456789);
  expect(15, ldt.hourOfDay);
  expect(3, ldt.hourOf12HourClock);
  expect(48, ldt.minuteOfHour);
...
void Operators_DifferentCalendars_Throws() {
  ...
  LocalDateTime value2 =
      LocalDateTime(2011, 1, 3, 10, 30, 0, calendar: CalendarSystem.julian);
...
void CompareTo_DifferentCalendars_Throws() {
  ...
  LocalDateTime value1 = LocalDateTime(2011, 1, 2, 10, 30, 0);
  LocalDateTime value2 =
      LocalDateTime(1500, 1, 1, 10, 30, 0, calendar: islamic);
...
void InZone() {
  ...
  expect(ZonedDateTime.atLeniently(ambiguous, Pacific),
      ambiguous.inZone(Pacific, Resolvers.lenientResolver));
  expect(ZonedDateTime.atLeniently(skipped, Pacific),
      skipped.inZone(Pacific, Resolvers.lenientResolver));
}
...
void MinMax_DifferentCalendars_Throws() {
  ...
  LocalDateTime ldt2 =
      LocalDateTime(1500, 1, 1, 5, 10, 0, calendar: CalendarSystem.julian);
...
void MinMax_SameCalendar() {
  LocalDateTime ldt1 =
      LocalDateTime(1500, 1, 1, 7, 20, 0, calendar: CalendarSystem.julian);
  LocalDateTime ldt2 =
      LocalDateTime(1500, 1, 1, 5, 10, 0, calendar: CalendarSystem.julian);
