...
void AllMonths() {
  ...
  for (int month = 1; month <= 32; month++) {
    var ymdc =
        YearMonthDayCalendar(-123, month, 20, CalendarOrdinal.hebrewCivil);
...
void AllDays() {
  ...
  for (int day = 1; day <= 64; day++) {
    var ymdc = YearMonthDayCalendar(
        -123, 12, day, CalendarOrdinal.islamicAstronomicalBase15);
    expect(-123, ymdc.year);
...
void AllCalendars() {
  for (int ordinal = 0; ordinal < 64; ordinal++) {
    CalendarOrdinal calendar =
        CalendarOrdinal(ordinal); //(CalendarOrdinal) ordinal;
...
void Equality() {
  ...
  TestHelper.TestEqualsStruct(
      original, YearMonthDayCalendar(1000, 12, 20, CalendarOrdinal.coptic), [
    YearMonthDayCalendar(original.year + 1, original.month, original.day,
        original.calendarOrdinal),
    YearMonthDayCalendar(original.year, original.month + 1, original.day,
        original.calendarOrdinal),
    YearMonthDayCalendar(original.year, original.month, original.day + 1,
        original.calendarOrdinal),
    YearMonthDayCalendar(
        original.year, original.month, original.day, CalendarOrdinal.gregorian)
  ]);
...
void Parse(
    String text, int year, int month, int day, CalendarOrdinal calendar) {
