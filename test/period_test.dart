...
final PeriodUnits HoursMinutesPeriodType =
    PeriodUnits.hours | PeriodUnits.minutes;

final List<PeriodUnits> AllPeriodUnits = PeriodUnits.values;

@Test()
void BetweenLocalDateTimes_WithoutSpecifyingUnits_OmitsWeeks() {
  Period actual = Period.differenceBetweenDateTime(
      LocalDateTime(2012, 2, 21, 0, 0, 0), LocalDateTime(2012, 2, 28, 0, 0, 0));
...
void BetweenLocalDateTimes_MovingForwardWithAllFields_GivesExactResult() {
  Period actual =
      Period.differenceBetweenDateTime(TestDateTime1, TestDateTime2);
  Period expected = const Period(hours: 2) +
      const Period(minutes: 14) +
      const Period(seconds: 55);
  expect(expected, actual);
}
...
void BetweenLocalDateTimes_MovingBackwardWithAllFields_GivesExactResult() {
  Period actual =
      Period.differenceBetweenDateTime(TestDateTime2, TestDateTime1);
  Period expected = const Period(hours: -2) +
      const Period(minutes: -14) +
      const Period(seconds: -55);
  expect(expected, actual);
...
@Test()
void
    BetweenLocalDateTimes_MovingForwardWithHoursAndMinutes_RoundsTowardsStart() {
  Period actual = Period.differenceBetweenDateTime(
      TestDateTime1, TestDateTime2, HoursMinutesPeriodType);
...
@Test()
void
    BetweenLocalDateTimes_MovingBackwardWithHoursAndMinutes_RoundsTowardsStart() {
  Period actual = Period.differenceBetweenDateTime(
      TestDateTime2, TestDateTime1, HoursMinutesPeriodType);
  Period expected = const Period(hours: -2) + const Period(minutes: -14);
  expect(expected, actual);
}
...
void BetweenLocalDateTimes_AcrossDays() {
  ...
  Period actual = Period.differenceBetweenDateTime(
      TestDateTime1, TestDateTime1.addDays(1).addMinutes(-1));
  expect(expected, actual);
}
...
void BetweenLocalDateTimes_AcrossDays_MinutesAndSeconds() {
  Period expected =
      const Period(minutes: 24 * 60 - 1) + const Period(seconds: 59);
  Period actual = Period.differenceBetweenDateTime(
      TestDateTime1,
      TestDateTime1.addDays(1).addSeconds(-1),
...
void BetweenLocalDateTimes_NotInt64Representable() {
  ...
  expect(
      (ILocalDateTime.toLocalInstant(end).timeSinceLocalEpoch -
              ILocalDateTime.toLocalInstant(start).timeSinceLocalEpoch)
          .canNanosecondsBeInteger,
      isFalse);
  ...
  Period actual =
      Period.differenceBetweenDateTime(start, end, PeriodUnits.allTimeUnits);
...
void BetweenLocalDates_InvalidUnits() {
  expect(
      () => Period.differenceBetweenDates(
          TestDate1, TestDate2, const PeriodUnits(0)),
      throwsArgumentError);
  expect(
      () => Period.differenceBetweenDates(
          TestDate1, TestDate2, const PeriodUnits(-1)),
      throwsArgumentError);
  expect(
      () => Period.differenceBetweenDates(
          TestDate1, TestDate2, PeriodUnits.allTimeUnits),
      throwsArgumentError);
  expect(
      () => Period.differenceBetweenDates(
          TestDate1, TestDate2, PeriodUnits.years | PeriodUnits.hours),
      throwsArgumentError);
}
...
void BetweenLocalDates_SingleUnit(
    String startText, String endText, PeriodUnits units, int expectedValue) {
...
void BetweenLocalDates_MovingForwardInLeapYear_WithExactResults() {
  ...
  Period expected =
      const Period(years: 1) + const Period(months: 8) + const Period(days: 11);
  expect(expected, actual);
}
...
void BetweenLocalDates_MovingBackwardInLeapYear_WithExactResults() {
  ...
  Period expected = const Period(years: -1) +
      const Period(months: -8) +
      const Period(days: -12);
  expect(expected, actual);
}
...
void BetweenLocalDates_MovingForward_WithJustMonths() {
  Period actual =
      Period.differenceBetweenDates(TestDate1, TestDate3, PeriodUnits.months);
...
void BetweenLocalDates_MovingBackward_WithJustMonths() {
  Period actual =
      Period.differenceBetweenDates(TestDate3, TestDate1, PeriodUnits.months);
  Period expected = const Period(months: -20);
  expect(expected, actual);
}
...
void BetweenLocalDates_AssymetricForwardAndBackward() {
  ...
  expect(const Period(months: 1) + const Period(days: 20),
  ...
  expect(const Period(months: -1) + const Period(days: -18),
...
void BetweenLocalDates_OnLeapYear() {
  ...
  expect(const Period(months: -11) + const Period(days: -28),
      Period.differenceBetweenDates(d2, d1));
}
...
void BetweenLocalTimes_InvalidUnits() {
  ...
  LocalTime t2 = LocalTime(15, 30, 45,
      ns: 20 * TimeConstants.nanosecondsPerMillisecond + 5 * 100);
  expect(() => Period.differenceBetweenTimes(t1, t2, const PeriodUnits(0)),
      throwsArgumentError);
  expect(() => Period.differenceBetweenTimes(t1, t2, const PeriodUnits(-1)),
      throwsArgumentError);
  expect(() => Period.differenceBetweenTimes(t1, t2, PeriodUnits.yearMonthDay),
      throwsArgumentError);
  expect(
      () => Period.differenceBetweenTimes(
          t1, t2, PeriodUnits.years | PeriodUnits.hours),
      throwsArgumentError);
}
...
void BetweenLocalTimes_SingleUnit(
    String startText, String endText, PeriodUnits units, int expectedValue) {
...
void BetweenLocalTimes_MovingForwards() {
  ...
  LocalTime t2 = LocalTime(15, 30, 45,
      ns: 20 * TimeConstants.nanosecondsPerMillisecond + 5 * 100);
  expect(
      const Period(hours: 5) +
          const Period(minutes: 30) +
          const Period(seconds: 45) +
          const Period(milliseconds: 20) +
          const Period(nanoseconds: 500),
      Period.differenceBetweenTimes(t1, t2));
}

@Test()
void BetweenLocalTimes_MovingBackwards() {
  LocalTime t1 = LocalTime(15, 30, 45,
      ns: 20 * TimeConstants.nanosecondsPerMillisecond + 5 * 100);
...
void BetweenLocalTimes_MovingForwards_WithJustHours() {
  ...
  expect(const Period(hours: 5),
      Period.differenceBetweenTimes(t1, t2, PeriodUnits.hours));
}
...
void Equality_WithDifferentPeriodTypes_OnlyConsidersValues() {
  Period allFields = const Period(minutes: 1) +
      const Period(hours: 1) -
      const Period(minutes: 1);
...
void HasTimeComponent_Compound() {
  ...
  expect(
      Period.differenceBetweenDates(dt1.calendarDate, dt2.calendarDate)
          .hasTimeComponent,
      isFalse);

  // Case 2: Period contains date and time units, but time units are all zero
  expect(
      Period.differenceBetweenDateTime(dt1.calendarDate.at(LocalTime.midnight),
              dt2.calendarDate.at(LocalTime.midnight))
          .hasTimeComponent,
      isFalse);

  // Case 3: Entire period is time-based, but 0. (Same local time twice here.)
  expect(
      Period.differenceBetweenTimes(dt1.clockTime, dt1.clockTime)
          .hasTimeComponent,
      isFalse);
  ...
  expect(
      Period.differenceBetweenTimes(dt1.clockTime, dt2.clockTime)
          .hasTimeComponent,
      isTrue);
}
...
void HasDateComponent_Compound() {
  ...
  expect(
      Period.differenceBetweenDateTime(dt1, dt1.calendarDate.at(dt2.clockTime))
          .hasDateComponent,
      isFalse);

  // Case 3: Entire period is date-based, but 0. (Same local date twice here.)
  expect(
      Period.differenceBetweenDates(dt1.calendarDate, dt1.calendarDate)
          .hasDateComponent,
      isFalse);
...
void ToDuration_ValidAllAcceptableUnits() {
  ...
  expect(
      1 * TimeConstants.microsecondsPerWeek +
          ...
          6 * TimeConstants.microsecondsPerMillisecond +
          7,
      period.toTime().totalMicroseconds);
}
...
void NormalizingEqualityComparer_PeriodToItself() {
  Period period = const Period(years: 1);
  expect(NormalizingPeriodEqualityComparer.instance.equals(period, period),
      isTrue);
}
...
void NormalizingEqualityComparer_NonEqualAfterNormalization() {
  ...
  expect(NormalizingPeriodEqualityComparer.instance.equals(period1, period2),
      isFalse);
}
...
void Comparer_NonDurationablePeriods() {
  ...
  var januaryComparer =
      Period.createComparer(LocalDateTime(2000, 1, 1, 0, 0, 0));
  ...
  var februaryComparer =
      Period.createComparer(LocalDateTime(2000, 2, 1, 0, 0, 0));
...
void Between_ExtremeValues_Overflow() {
  ...
  expect(
      () => Period.differenceBetweenDateTime(
          minValue, maxValue, PeriodUnits.nanoseconds),
      throwsRangeError); // throwsStateError);
}
...
@TestCase(
    ['2014-01-01T16:00:00', "2014-01-03T08:00:00", PeriodUnits.days, 1, -1])
@TestCase(
    ['2014-01-01T16:00:00', "2014-01-03T08:00:00", PeriodUnits.hours, 40, -40])
void Between_LocalDateTime_AwkwardTimeOfDayWithSingleUnit(
    String startText,
    String endText,
    PeriodUnits units,
    int expectedForward,
...
void Between_LocalDateTime_AwkwardTimeOfDayWithMultipleUnits() {
  ...
  Period actual = Period.differenceBetweenDateTime(
      start, end, PeriodUnits.yearMonthDay | PeriodUnits.allTimeUnits);
  Period expected = (PeriodBuilder()
        ..years = 1
        ..months = 1
        ..days = 1
