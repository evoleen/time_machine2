...
void setFunctions() {
  ...
  testOperatorComparisonEqualityFunction =
      TestHelperWithMirrors.TestOperatorComparisonEquality;
...
// print(instanceMirror.invoke(lt.simpleName, [null]).reflectee);
    ...
    if (greaterThan != null) {
      ...
      {
        expect(valueMirror.invoke(gt, [null]).reflectee, isTrue,
            reason: 'value > null');
        // expect(greaterThan.Invoke(null, [ null, value ], isFalse, reason: 'null > value');
      }
      expect(valueMirror.invoke(gt, [value]).reflectee, isFalse,
          reason: 'value > value');
      expect(valueMirror.invoke(gt, [equalValue]).reflectee, isFalse,
          reason: 'value > equalValue');
      expect(equalValueMirror.invoke(gt, [value]).reflectee, isFalse,
          reason: 'equalValue > value');
    }
    if (lessThan != null) {
      // if (!type.GetTypeInfo().IsValueType)
      {
        expect(valueMirror.invoke(lt, [null]).reflectee, isFalse,
            reason: 'value < null');
        // expect(lessThan.Invoke(null, [ null, value ], isTrue, reason: 'null < value');
      }
      expect(valueMirror.invoke(lt, [value]).reflectee, isFalse,
          reason: 'value > value');
      expect(valueMirror.invoke(lt, [equalValue]).reflectee, isFalse,
          reason: 'value > equalValue');
      expect(equalValueMirror.invoke(lt, [value]).reflectee, isFalse,
          reason: 'equalValue > value');
    }
    ...
    for (var greaterValue in greaterValues) {
      ...
      if (greaterThan != null) {
        expect(valueMirror.invoke(gt, [greaterValue]).reflectee, isFalse,
            reason: 'value > greaterValue');
        expect(greaterValueMirror.invoke(gt, [value]).reflectee, isTrue,
            reason: 'greaterValue > value');
      }
      if (lessThan != null) {
        expect(valueMirror.invoke(lt, [greaterValue]).reflectee, isTrue,
            reason: 'value < greaterValue');
        expect(greaterValueMirror.invoke(lt, [value]).reflectee, isFalse,
            reason: 'greaterValue < value');
      }
  ...
  static void TestOperatorComparisonEquality<T>(
    ...
    if (greaterThanOrEqual != null) {
      //if (!type.GetTypeInfo().IsValueType)
      {
        expect(valueMirror.invoke(gte, [null]).reflectee, isTrue,
            reason: 'value >= null');
        // expect(valueMirror.invoke(gte, [value]), greaterThanOrEqual.Invoke(null, [ null, value ], isFalse, reason: 'null >= value');
      }
      expect(valueMirror.invoke(gte, [value]).reflectee, isTrue,
          reason: 'value >= value');
      expect(valueMirror.invoke(gte, [equalValue]).reflectee, isTrue,
          reason: 'value >= equalValue');
      expect(equalValueMirror.invoke(gte, [value]).reflectee, isTrue,
          reason: 'equalValue >= value');
    }
    if (lessThanOrEqual != null) {
      //if (!type.GetTypeInfo().IsValueType)
      {
        expect(valueMirror.invoke(lte, [null]).reflectee, isFalse,
            reason: 'value <= null');
        // expect(lessThanOrEqual.Invoke(null, [ null, value ], isTrue, reason: 'null <= value');
      }
      expect(valueMirror.invoke(lte, [value]).reflectee, isTrue,
          reason: 'value <= value');
      expect(valueMirror.invoke(lte, [equalValue]).reflectee, isTrue,
          reason: 'value <= equalValue');
      expect(equalValueMirror.invoke(lte, [value]).reflectee, isTrue,
          reason: 'equalValue <= value');
    }
    ...
    for (var greaterValue in greaterValues) {
      InstanceMirror greaterValueMirror = reflect(greaterValue);
      if (greaterThanOrEqual != null) {
        expect(valueMirror.invoke(gte, [greaterValue]).reflectee, isFalse,
            reason: 'value >= greaterValue');
        expect(greaterValueMirror.invoke(gte, [value]).reflectee, isTrue,
            reason: 'greaterValue >= value');
      }
      if (lessThanOrEqual != null) {
        expect(valueMirror.invoke(lte, [greaterValue]).reflectee, isTrue,
            reason: 'value <= greaterValue');
        expect(greaterValueMirror.invoke(lte, [value]).reflectee, isFalse,
            reason: 'greaterValue <= value');
      }
      // Now move up to the next pair...
      value = greaterValue;
  ...
  static void TestOperatorEquality<T>(T value, T equalValue, T unequalValue) {
    ...
    if (equality != null) {
      // if (!type.GetTypeInfo().IsValueType)
      {
        ...
        expect(valueMirror.invoke(equ, [null]).reflectee, isFalse,
            reason: 'value == null');
        // expect(equality.Invoke(null, [ null, value ], isFalse, reason: 'null == value');
      }
      expect(valueMirror.invoke(equ, [value]).reflectee, isTrue,
          reason: 'value == value');
      expect(valueMirror.invoke(equ, [equalValue]).reflectee, isTrue,
          reason: 'value == equalValue');
      expect(equalValueMirror.invoke(equ, [value]).reflectee, isTrue,
          reason: 'equalValue == value');
      expect(valueMirror.invoke(equ, [unequalValue]).reflectee, isFalse,
          reason: 'value == unequalValue');
    }
  }
}
