...
final SingleTransitionDateTimeZone TestZone =
    SingleTransitionDateTimeZone.around(Instant.utc(2000, 1, 1, 0, 0), -3, 4);
...
void GetZoneIntervals_EndBeforeStart() {
  ...
  expect(
      () => DateTimeZone.utc.getZoneIntervalsFromTo(
          Instant.fromEpochMicroseconds(10),
          Instant().add(Time(nanoseconds: 9900))),
      throwsArgumentError);
}
...
void GetZoneIntervals_EndEqualToStart() {
  expect(
      DateTimeZone.utc.getZoneIntervalsFromTo(
          Instant.fromEpochMicroseconds(10), Instant.fromEpochMicroseconds(10)),
      isEmpty);
}
...
void GetZoneIntervals_InvalidOptions() {
  ...
  var interval =
      Interval(Instant.utc(2000, 1, 1, 0, 0), Instant.utc(2001, 1, 1, 0, 0));
  // Assert.Throws<ArgumentOutOfRangeException>(() => zone.GetZoneIntervals(interval, (ZoneEqualityComparer.Options) 1234567));
  expect(
      () => zone.getZoneIntervalsOptions(
          interval, const ZoneEqualityComparerOptions(1234567)),
      throwsArgumentError);
}
...
void GetZoneIntervals_FixedZone() {
  ...
  var actual = zone.getZoneIntervalsFromTo(
      Instant.utc(1900, 1, 1, 0, 0), Instant.utc(2100, 1, 1, 0, 0));
...
void GetZoneIntervals_WithOptions_NoCoalescing() {
  ...
  var interval =
      Interval(Instant.utc(1999, 1, 1, 0, 0), Instant.utc(2003, 1, 1, 0, 0));
  // No coalescing required, as the names are different.
  var zoneIntervals = zone
      .getZoneIntervalsOptions(interval, ZoneEqualityComparerOptions.matchNames)
      .toList();
  ...
  expect([transition1, transition2, transition3, transition4],
      zoneIntervals.map((zi) => zi.end));
}
...
void GetZoneIntervals_WithOptions_Coalescing() {
  ...
  var zoneIntervals = zone
      .getZoneIntervalsOptions(
          interval, ZoneEqualityComparerOptions.onlyMatchWallOffset)
      .toList();
  expect(3, zoneIntervals.length);
  // CollectionAssert.AreEqual
  expect([transition1, transition3, transition4],
      zoneIntervals.map((zi) => zi.end));
  expect([IInstant.beforeMinValue, transition1, transition3],
      zoneIntervals.map((zi) => IZoneInterval.rawStart(zi)));
