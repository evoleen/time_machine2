...
void OperatorPlus_Zero_IsNeutralElement() {
  ...
  expect(TestObjects.CreatePositiveOffset(3, 0, 0), ThreeHours + Offset.zero,
      reason: 'ThreeHours + 0');
  expect(TestObjects.CreatePositiveOffset(3, 0, 0), Offset.zero + ThreeHours,
      reason: '0 + ThreeHours');
}

@Test()
void OperatorPlus_NonZero() {
  expect(TestObjects.CreatePositiveOffset(6, 0, 0), ThreeHours + ThreeHours,
      reason: 'ThreeHours + ThreeHours');
  expect(Offset.zero, ThreeHours + NegativeThreeHours,
      reason: 'ThreeHours + (-ThreeHours)');
  expect(TestObjects.CreateNegativeOffset(9, 0, 0),
      NegativeTwelveHours + ThreeHours,
      reason: '-TwelveHours + ThreeHours');
}

// Static method equivalents
@Test()
void MethodAdd_Zero_IsNeutralElement() {
  expect(0, Offset.plus(Offset.zero, Offset.zero).inMilliseconds,
      reason: '0 + 0');
  expect(TestObjects.CreatePositiveOffset(3, 0, 0),
      Offset.plus(ThreeHours, Offset.zero),
      reason: 'ThreeHours + 0');
  expect(TestObjects.CreatePositiveOffset(3, 0, 0),
      Offset.plus(Offset.zero, ThreeHours),
      reason: '0 + ThreeHours');
}

@Test()
void MethodAdd_NonZero() {
  expect(TestObjects.CreatePositiveOffset(6, 0, 0),
      Offset.plus(ThreeHours, ThreeHours),
      reason: 'ThreeHours + ThreeHours');
  expect(Offset.zero, Offset.plus(ThreeHours, NegativeThreeHours),
      reason: 'ThreeHours + (-ThreeHours)');
  expect(TestObjects.CreateNegativeOffset(9, 0, 0),
      Offset.plus(NegativeTwelveHours, ThreeHours),
      reason: '-TwelveHours + ThreeHours');
}
...
void MethodPlus_Zero_IsNeutralElement() {
  ...
  expect(TestObjects.CreatePositiveOffset(3, 0, 0), ThreeHours.add(Offset.zero),
      reason: 'ThreeHours + 0');
  expect(TestObjects.CreatePositiveOffset(3, 0, 0), Offset.zero.add(ThreeHours),
      reason: '0 + ThreeHours');
}

@Test()
void MethodPlus_NonZero() {
  expect(TestObjects.CreatePositiveOffset(6, 0, 0), ThreeHours.add(ThreeHours),
      reason: 'ThreeHours + ThreeHours');
  expect(Offset.zero, ThreeHours.add(NegativeThreeHours),
      reason: 'ThreeHours + (-ThreeHours)');
  expect(TestObjects.CreateNegativeOffset(9, 0, 0),
      NegativeTwelveHours.add(ThreeHours),
      reason: '-TwelveHours + ThreeHours');
}
// #endregion
...
void OperatorMinus_Zero_IsNeutralElement() {
  ...
  expect(TestObjects.CreatePositiveOffset(3, 0, 0), ThreeHours - Offset.zero,
      reason: 'ThreeHours - 0');
  expect(TestObjects.CreateNegativeOffset(3, 0, 0), Offset.zero - ThreeHours,
      reason: '0 - ThreeHours');
}

@Test()
void OperatorMinus_NonZero() {
  expect(Offset.zero, ThreeHours - ThreeHours,
      reason: 'ThreeHours - ThreeHours');
  expect(TestObjects.CreatePositiveOffset(6, 0, 0),
      ThreeHours - NegativeThreeHours,
      reason: 'ThreeHours - (-ThreeHours)');
  expect(TestObjects.CreateNegativeOffset(15, 0, 0),
      NegativeTwelveHours - ThreeHours,
      reason: '-TwelveHours - ThreeHours');
}
...
void Subtract_Zero_IsNeutralElement() {
  ...
  expect(TestObjects.CreatePositiveOffset(3, 0, 0),
      Offset.minus(ThreeHours, Offset.zero),
      reason: 'ThreeHours - 0');
  expect(TestObjects.CreateNegativeOffset(3, 0, 0),
      Offset.minus(Offset.zero, ThreeHours),
      reason: '0 - ThreeHours');
}

@Test()
void Subtract_NonZero() {
  expect(Offset.zero, Offset.minus(ThreeHours, ThreeHours),
      reason: 'ThreeHours - ThreeHours');
  expect(TestObjects.CreatePositiveOffset(6, 0, 0),
      Offset.minus(ThreeHours, NegativeThreeHours),
      reason: 'ThreeHours - (-ThreeHours)');
  expect(TestObjects.CreateNegativeOffset(15, 0, 0),
      Offset.minus(NegativeTwelveHours, ThreeHours),
      reason: '-TwelveHours - ThreeHours');
}
...
void Minus_Zero_IsNeutralElement() {
  ...
  expect(TestObjects.CreatePositiveOffset(3, 0, 0),
      ThreeHours.subtract(Offset.zero),
      reason: 'ThreeHours - 0');
  expect(TestObjects.CreateNegativeOffset(3, 0, 0),
      Offset.zero.subtract(ThreeHours),
      reason: '0 - ThreeHours');
}

@Test()
void Minus_NonZero() {
  expect(Offset.zero, ThreeHours.subtract(ThreeHours),
      reason: 'ThreeHours - ThreeHours');
  expect(TestObjects.CreatePositiveOffset(6, 0, 0),
      ThreeHours.subtract(NegativeThreeHours),
      reason: 'ThreeHours - (-ThreeHours)');
  expect(TestObjects.CreateNegativeOffset(15, 0, 0),
      NegativeTwelveHours.subtract(ThreeHours),
      reason: '-TwelveHours - ThreeHours');
}
// #endregion
