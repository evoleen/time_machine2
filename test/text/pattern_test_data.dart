...
abstract class PatternTestData<T> {
  ...
  @internal
  late String standardPatternCode;

  /// Pattern text.
  @internal
  String pattern = '***undefined pattern***';
  ...
  @internal
  PatternTestData(this.Value) {
    template = defaultTemplate;
  }

  @internal
  IPattern<T> CreatePattern();
  ...
  void TestParse() {
    ...
    if (standardPattern != null) {
      assert(Value == standardPattern!.parse(text).value);
    }
  }
  ...
  @internal
  /*virtual*/ IPartialPattern<T> CreatePartialPattern() {
  ...
  @private
  static String FormatMessage(String message, List parameters) {
    try {
      return stringFormat(message, parameters);
    } on FormatException // catch ()
    {
      throw FormatException(
          "Failed to format String '$message' with ${parameters.length} parameters");
    }
  }
}
