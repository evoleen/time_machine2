...
final ZoneInterval firstInterval = IZoneInterval.newZoneInterval(
    'First',
    IInstant.beforeMinValue,
    Instant.utc(2000, 3, 10, 10, 0),
    Offset.hours(3),
    Offset.zero);

// Note that this is effectively UTC +3 + 1 hour DST.
final ZoneInterval secondInterval = IZoneInterval.newZoneInterval(
    'Second',
    firstInterval.end,
    Instant.utc(2000, 9, 15, 5, 0),
    Offset.hours(4),
    Offset.hours(1));

final ZoneInterval thirdInterval = IZoneInterval.newZoneInterval(
    'Third',
    secondInterval.end,
    Instant.utc(2005, 1, 20, 8, 0),
    Offset.hours(-5),
    Offset.zero);

final ZoneRecurrence winter = ZoneRecurrence(
    'Winter',
    Offset.zero,
    ZoneYearOffset(TransitionMode.wall, 10, 5, 0, false, LocalTime(2, 0, 0)),
    1960,
    Platform.int32MaxValue);

final ZoneRecurrence summer = ZoneRecurrence(
    'Summer',
    Offset.hours(1),
    ZoneYearOffset(TransitionMode.wall, 3, 10, 0, false, LocalTime(1, 0, 0)),
    1960,
    Platform.int32MaxValue);
...
final ZoneInterval clampedTailZoneInterval = IZoneInterval.withStart(
    tailZone.getZoneInterval(thirdInterval.end), thirdInterval.end)!;

final PrecalculatedDateTimeZone TestZone = PrecalculatedDateTimeZone(
    'Test', [firstInterval, secondInterval, thirdInterval], tailZone);
...
void MinMaxOffsetsWithOtherTailZone() {
  ...
  var testZone = PrecalculatedDateTimeZone(
      'Test', [firstInterval, secondInterval, thirdInterval], tailZone);
...
void MinMaxOffsetsWithNullTailZone() {
  var testZone = PrecalculatedDateTimeZone(
      ...
      [
        ...
        secondInterval,
        thirdInterval,
        IZoneInterval.newZoneInterval('Last', thirdInterval.end,
            IInstant.afterMaxValue, Offset.zero, Offset.zero)
      ],
      null);
  expect(Offset.hours(-5), testZone.minOffset);
  expect(Offset.hours(4), testZone.maxOffset);
...
void GetZoneIntervalInstant_End() {
  expect(secondInterval,
      TestZone.getZoneInterval(secondInterval.end - Time.epsilon));
}
...
void GetZoneIntervalInstant_FinalInterval_End() {
  expect(thirdInterval,
      TestZone.getZoneInterval(thirdInterval.end - Time.epsilon));
}
...
void MapLocal_UnambiguousInPrecalculated() {
  CheckMapping(
      LocalDateTime(2000, 6, 1, 0, 0, 0), secondInterval, secondInterval, 1);
}

@Test()
void MapLocal_UnambiguousInTailZone() {
  CheckMapping(LocalDateTime(2005, 2, 1, 0, 0, 0), clampedTailZoneInterval,
      clampedTailZoneInterval, 1);
}
...
void MapLocal_AmbiguousAroundTailZoneTransition() {
  ...
  CheckMapping(thirdInterval.isoLocalEnd.addNanoseconds(-1), thirdInterval,
      clampedTailZoneInterval, 2);
}
...
void MapLocal_AmbiguousButTooEarlyInTailZoneTransition() {
  ...
  var tailZone = SingleTransitionDateTimeZone.around(
      thirdInterval.end + Time(hours: 1), 10, 8);
  var gapZone = PrecalculatedDateTimeZone(
      'Test', [firstInterval, secondInterval, thirdInterval], tailZone);
...
void MapLocal_GapAroundTailZoneTransition() {
  ...
  expect(
      ...
      IZoneInterval.newZoneInterval('UTC+05', thirdInterval.end,
          IInstant.afterMaxValue, Offset.hours(5), Offset.zero));
...
void MapLocal_GapAroundAndInTailZoneTransition() {
  ...
  var tailZone = SingleTransitionDateTimeZone.around(
      thirdInterval.end + Time(hours: 1), -10, 5);
  var gapZone = PrecalculatedDateTimeZone(
      'Test', [firstInterval, secondInterval, thirdInterval], tailZone);
  var mapping = gapZone.mapLocal(thirdInterval.isoLocalEnd.addHours(1));
  expect(thirdInterval, mapping.earlyInterval);
  expect(
      IZoneInterval.newZoneInterval('Single-Early', thirdInterval.end,
          tailZone.Transition, Offset.hours(-10), Offset.zero),
      mapping.lateInterval);
  expect(0, mapping.count);
}
...
void GetZoneIntervals_NullTailZone_Eot() {
  List<ZoneInterval> intervals = [
    IZoneInterval.newZoneInterval('foo', IInstant.beforeMinValue,
        Instant.fromEpochMicroseconds(2), Offset.zero, Offset.zero),
    IZoneInterval.newZoneInterval('foo', Instant.fromEpochMicroseconds(2),
        IInstant.afterMaxValue, Offset.zero, Offset.zero)
  ];
...
void CheckMapping(LocalDateTime localDateTime, ZoneInterval earlyInterval,
...
void Validation_EmptyPeriodArray() {
  ...
  expect(
      () => PrecalculatedDateTimeZone.validatePeriods(
          [] /*new ZoneInterval[0]*/, DateTimeZone.utc),
      throwsArgumentError);
}
...
void Validation_BadFirstStartingPoint() {
  List<ZoneInterval> intervals = [
    IZoneInterval.newZoneInterval('foo', Instant.fromEpochMicroseconds(1),
        Instant.fromEpochMicroseconds(2), Offset.zero, Offset.zero),
    IZoneInterval.newZoneInterval('foo', Instant.fromEpochMicroseconds(2),
        Instant.fromEpochMicroseconds(3), Offset.zero, Offset.zero)
  ];
  // Assert.Throws<ArgumentException>
  expect(
      () => PrecalculatedDateTimeZone.validatePeriods(
          intervals, DateTimeZone.utc),
      throwsArgumentError);
}
...
void Validation_NonAdjoiningIntervals() {
  List<ZoneInterval> intervals = [
    ...
    IZoneInterval.newZoneInterval(
        'foo',
        Instant.fromEpochMicroseconds(2).add(Time(nanoseconds: 500)),
        Instant.fromEpochMicroseconds(3),
        Offset.zero,
        Offset.zero)
  ];
  // Assert.Throws<ArgumentException>
...
void Validation_Success() {
  List<ZoneInterval> intervals = [
    ...
    IZoneInterval.newZoneInterval('foo', Instant.fromEpochMicroseconds(2),
        Instant.fromEpochMicroseconds(3), Offset.zero, Offset.zero),
    IZoneInterval.newZoneInterval('foo', Instant.fromEpochMicroseconds(3),
        Instant.fromEpochMicroseconds(10), Offset.zero, Offset.zero),
    IZoneInterval.newZoneInterval('foo', Instant.fromEpochMicroseconds(10),
        Instant.fromEpochMicroseconds(20), Offset.zero, Offset.zero)
  ];
...
void Validation_NullTailZoneWithMiddleOfTimeFinalPeriod() {
  ...
  // Assert.Throws<ArgumentException>
  expect(() => PrecalculatedDateTimeZone.validatePeriods(intervals, null),
      throwsArgumentError);
}
